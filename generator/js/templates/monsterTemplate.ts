...
function card_element_subtitle(text) {
    return '<div class="card-element card-subtitle">' + (text || "") + '</div>';
}
...
function card_element_ruler(card_data, options) {
    var color = card_data_color_front(card_data, options);
    var fill = 'fill="' + color + '"';
...
function card_element_property(name, values) {
    var result = "";
    if(values){
        result += '<div class="card-element card-property-line">';
        result += '   <h4 class="card-property-name">' + name + '</h4>';
        result += '   <p class="card-p card-property-text">' + values[0] + '</p>';
        if (values[1])
        {
            result += '   <div style="float:right">';
            result += '       <h4 class="card-property-name">' + values[1] + '</h4>';
            result += '       <p class="card-p card-property-text">' + values[2] + '</p>';
            result += '   </div>';
        }
        result += '</div>';
    }
...
function card_generate_contents(monster:Monster, card_data, options) {
    var result = "";
    result += '<div class="card-content-container">';
    card_element_subtitle(monster.getSpeciesSumary());
    card_element_ruler(card_data, options);
    card_element_property("Armor class", monster.AC);
    card_element_property("Hit points", monster.HP);
    card_element_property("Speed", monster.speed);
    card_element_ruler(card_data, options);
    card_element_property("Skills", monster.skills);
    card_element_property("Senses", monster.senses);
    card_element_property("Language", monster.languages);
    card_element_property("Speed", monster.speed);
...
function card_generate_front(data:Monster, options) {
    ...
    result += card_element_title(data.name, options);
    result += card_generate_contents(data, data, options);
    result += '</div>';
